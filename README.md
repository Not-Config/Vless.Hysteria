...
Это вариант «сайт и VPN на одном сервере». Для проверки поведения белых списков можно использовать домен, которым вы управляете и который действительно разрешён в тестируемой сети. Просто подставлять чужое разрешённое имя недостаточно и не гарантирует работу.
...
подходит для лаборатории. Hysteria2 получает `pinSHA256`, а для `web-grpc` при необходимости используется `allowInsecure=1`.
...
В режиме Let's Encrypt клиентская ссылка Hysteria2 больше не привязывается к SHA-256 конкретного сертификата, поэтому обычное продление сертификата не ломает клиента. Для VLESS `web-grpc` также не добавляется `allowInsecure`.
...
Таким образом, обычный HTTP/3-запрос получает реальный контент, а не только заглушку. В сочетании с одним доменом и публично доверенным сертификатом TCP/HTTPS и UDP/HTTP3 относятся к одному и тому же сайту.

Это не гарантирует прохождение «белого списка»: сеть может независимо фильтровать UDP/QUIC, проверять destination IP, ASN и другие признаки. Такой режим нужен именно для согласованной и проверяемой конфигурации, а не для предположения, что одного SNI достаточно.
